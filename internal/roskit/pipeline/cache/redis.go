...
type Repository interface {
	...
	DeleteByMeasurement(ctx context.Context, routerID, measurement string) error
...
func (r *RedisRepository) DeleteByMeasurement(ctx context.Context, routerID, measurement string) error {
	pattern := fmt.Sprintf("roskit:%s:%s:*", routerID, measurement)
	var cursor uint64
	for {
		keys, nextCursor, err := r.client.Scan(ctx, cursor, pattern, 100).Result()
		...
		if len(keys) > 0 {
			if err := r.client.Del(ctx, keys...).Err(); err != nil {
				return fmt.Errorf("redis: DEL %s: %w", pattern, err)
			}
		}
		cursor = nextCursor
		if cursor == 0 {
...
func (NoopRepository) DeleteByMeasurement(_ context.Context, _, _ string) error { return nil }
