...
// FirewallNATSpec streams NAT rule changes and hit counters.
// Uses =stats mode for real-time bytes/packets counters per rule.
type FirewallNATSpec struct{}

func NewFirewallNATSpec() *FirewallNATSpec { return &FirewallNATSpec{} }

func (s *FirewallNATSpec) Command() []string {
	return []string{
...
func (s *FirewallNATSpec) Tag() string         { return "firewall-nat" }
func (s *FirewallNATSpec) Measurement() string { return "firewall_nat" }

func (s *FirewallNATSpec) Parse(routerID string, sentence *proto.Sentence) (*domain.TelemetryEvent, error) {
	pairs := sentence.Map
	if len(pairs) == 0 {
		return nil, nil
	}
