...
// FirewallMangleSpec streams firewall mangle rule counters.
// Used for traffic classification and QoS marking monitoring.
type FirewallMangleSpec struct{}

func NewFirewallMangleSpec() *FirewallMangleSpec { return &FirewallMangleSpec{} }

func (s *FirewallMangleSpec) Command() []string {
	return []string{
...
func (s *FirewallMangleSpec) Tag() string         { return "firewall-mangle" }
func (s *FirewallMangleSpec) Measurement() string { return "firewall_mangle" }

func (s *FirewallMangleSpec) Parse(routerID string, sentence *proto.Sentence) (*domain.TelemetryEvent, error) {
	pairs := sentence.Map
	if len(pairs) == 0 {
		return nil, nil
	}
