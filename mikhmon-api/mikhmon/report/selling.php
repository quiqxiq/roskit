...
if (!isset($_SESSION["mikhmon"])) {
	...
	// Ambil daftar server untuk dropdown filter
...
<div class="row">
	...
	<div class="col-12">
		...
		<!-- Filter Server -->
		<form method="get" style="display:inline-block; margin-bottom:5px;">
		  <input type="hidden" name="session" value="<?= $session ?>">
		  <?php if($idhr != ''){echo '<input type="hidden" name="idhr" value="'.$idhr.'">';}?>
		  <?php if($idbl != ''){echo '<input type="hidden" name="idbl" value="'.$idbl.'">';}?>
		  <select name="server" onchange="this.form.submit()" class="form-control" style="display:inline-block; width:auto; padding:3px 6px;">
			...
			if ($prefix != "") {
				for ($i = 0; $i < $TotalReg; $i++) {
					...
					if (substr($getname[2], 0, strlen($prefix)) == $prefix) {
						...
						echo "</td>";
						echo "<td>";
						echo htmlspecialchars($rowServer);
						echo "</td>";
